...
bool HitableHit(const in Hitable hb, const in Ray r, const in float tmin, const in float tmax, inout RayHit hit)
{
	...
	if (discriminant > 0.0)
	{
		...
		if (t < tmax && t > tmin)
		{
			...
			hit.Normal = (hit.Position - hb.Center) / hb.Radius;
			return true;
		}
	}
	else
...
float3 ComputeColor(in Ray r, inout uint seed)
{
	...
	for (uint depth = 0; depth < data.recursion; depth++)
	{
		...
		{
			RayHit tempHit;
			if (HitableHit(Spheres[i], r, 0.001, tmax, tempHit))
			{
				hitAnything = true;
				hit = tempHit;
				hitID = i;
